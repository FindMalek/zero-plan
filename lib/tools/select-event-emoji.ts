...
export const selectEventEmojiTool = tool({
  ...
  inputSchema: z.object({
    ...
    eventTitle: z.string().describe("The full event title or description"),
    timeOfDay: z
      .string()
      .optional()
  ...
  execute: async ({
    ...
    const emojiAnalysis = await generateObject({
      ...
      schema: z.object({
        selectedEmoji: z
          .string()
          .describe("The most appropriate single emoji for this event"),
        primaryReason: z
          .enum([
            "activity_specific",
            "social_context",
        ...
        confidence: z
          ...
          .min(0)
          .max(1)
          .describe("Confidence in emoji selection (0-1)"),
        alternativeOptions: z
          .array(z.string())
          .max(3)
          .describe("Up to 3 alternative emoji options"),
        reasoning: z
          .string()
          .describe("Brief explanation of why this emoji was chosen"),
        contextFactors: z.object({
          activityType: z
            .string()
            .describe("What type of activity this represents"),
          socialAspect: z
            .string()
            .describe(
              "Social context (solo, with friends, professional, etc.)"
            ),
          culturalRelevance: z
            .string()
            .describe(
              "Any cultural considerations (Tunisian context, local customs)"
            ),
        }),
      }),
