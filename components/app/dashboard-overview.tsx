...
import { UserRo } from "@/config/schema"
...
export function DashboardOverview({
    ...
    const expiringCards = cards.filter((card) => {
      ...
      return (
        (cardYear === currentYear &&
          cardMonth - currentMonth <= 3 &&
          cardMonth - currentMonth >= 0) ||
  ...
  const getSecurityStatus = () => {
    if (securityScore >= 80)
      return { label: "Excellent", color: "text-green-500", icon: ShieldCheck }
    if (securityScore >= 60)
      return { label: "Good", color: "text-yellow-500", icon: Shield }
    return {
  ...
  const getExpiringCards = () => {
    ...
    return cards
      ...
      .sort((a, b) => {
        const aExpiry =
          Number.parseInt(a.expiryYear) * 12 + Number.parseInt(a.expiryMonth)
        const bExpiry =
          Number.parseInt(b.expiryYear) * 12 + Number.parseInt(b.expiryMonth)
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <securityStatus.icon
              className={`h-4 w-4 ${securityStatus.color}`}
            />
          </CardHeader>
          <CardContent>
            ...
            <p className="text-muted-foreground mt-2 text-xs">
              {weakPasswords.length > 0
                ? `${weakPasswords.length} weak passwords`
