...
async function seedInputProcessingSessions(prisma: PrismaClient) {
  ...
  for (const user of users) {
    const userEvents = events.filter((e) => e.userId === user.id && e.aiConfidence)
    ...
    for (const event of userEvents) {
      ...
      if (event.title.includes("Grocery Shopping")) {
        userInput = "I need to buy groceries this Saturday morning at 10am at Whole Foods"
        processedOutput = {
          events: [{
        ...
        userInput = "Schedule daily team standup meetings at 9am in conference room A with zoom link"
        processedOutput = {
          events: [{
        ...
        userInput = "Sarah's 30th birthday party this Saturday evening from 6pm to 10pm at her house"
        processedOutput = {
          events: [{
    ...
    if (user.email === "john@example.com") {
      processingsToCreate.push({
        ...
        processedOutput: JSON.stringify({ error: "Unable to extract clear event information" }),
        model: "gpt-4",
        provider: "openai",
        ...
        errorMessage: "Insufficient information to create event. User input was too vague.",
        createdAt: new Date(),
        updatedAt: new Date(),
        eventId: null,
        userId: user.id,
